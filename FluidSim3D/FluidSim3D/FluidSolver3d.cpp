...
FluidSolver3D::~FluidSolver3D(){
	// clean up grid
	deleteGrid3D<int>(m_gridWidth, m_gridHeight, m_gridDepth, m_label);
	deleteGrid3D<float>(m_gridWidth, m_gridHeight, m_gridDepth, m_p);
	deleteGrid3D<float>(m_gridWidth + 1, m_gridHeight, m_gridDepth, m_u);
	deleteGrid3D<float>(m_gridWidth + 1, m_gridHeight, m_gridDepth, m_uSaved);
	deleteGrid3D<float>(m_gridWidth, m_gridHeight + 1, m_gridDepth, m_v);
	deleteGrid3D<float>(m_gridWidth, m_gridHeight + 1, m_gridDepth, m_vSaved);
	deleteGrid3D<float>(m_gridWidth, m_gridHeight, m_gridDepth + 1, m_w);
	deleteGrid3D<float>(m_gridWidth, m_gridHeight, m_gridDepth + 1, m_wSaved);
	delete m_particles;
}
...
void FluidSolver3D::init(std::string initialGeometryFile){
	...
	m_label = initGrid3D<int>(m_gridWidth, m_gridHeight, m_gridDepth);
	m_p = initGrid3D<float>(m_gridWidth, m_gridHeight, m_gridDepth);
	m_u = initGrid3D<float>(m_gridWidth + 1, m_gridHeight, m_gridDepth);
	m_uSaved = initGrid3D<float>(m_gridWidth + 1, m_gridHeight, m_gridDepth);
	m_v = initGrid3D<float>(m_gridWidth, m_gridHeight + 1, m_gridDepth);
	m_vSaved = initGrid3D<float>(m_gridWidth, m_gridHeight + 1, m_gridDepth);
	m_w = initGrid3D<float>(m_gridWidth, m_gridHeight, m_gridDepth + 1);
	m_wSaved = initGrid3D<float>(m_gridWidth, m_gridHeight, m_gridDepth + 1);

	// init vel grids with unknown label value
	initGridValues<float>(m_u, m_gridWidth + 1, m_gridHeight, m_gridDepth, VEL_UNKNOWN);
	initGridValues<float>(m_v, m_gridWidth, m_gridHeight + 1, m_gridDepth, VEL_UNKNOWN);
	initGridValues<float>(m_w, m_gridWidth, m_gridHeight, m_gridDepth + 1, VEL_UNKNOWN);
	...
	readInGeom3D(m_gridWidth, m_gridHeight, m_gridDepth, m_dx, initialGeometryFile, m_label);
...
//*/
//void FluidSolver3D::pressureSolve() {
...
*/
template <typename T>
void FluidSolver3D::initGridValues(T*** grid, int x, int y, int z, T value) {
	for (int i = 0; i < x; i++) {
		for (int j = 0; j < y; j++) {
			for (int k = 0; k < z; k++) {
				grid[i][j][k] = value;
...
//given distance (x, y).
//*/
//double FluidSolver3D::quadBSplineKernel(SimUtil::Vec2) {
//	// TODO
//	return 0;
//}
//
...
//*/
//void FluidSolver3D::constructRHS(Mat2Dd rhs) {
//	// initialize to 0
...
//*/
//void FluidSolver3D::constructA(Mat2Dd Adiag, Mat2Dd Ax, Mat2Dd Ay) {
...
//*/
//void FluidSolver3D::constructPrecon(Mat2Dd precon, Mat2Dd Adiag, Mat2Dd Ax, Mat2Dd Ay) {
...
//*/
//void FluidSolver3D::PCG(Mat2Dd Adiag, Mat2Dd Ax, Mat2Dd Ay, Mat2Dd b, Mat2Dd precon) {
...
//*/
//void FluidSolver3D::applyPrecon(Mat2Dd z, Mat2Dd r, Mat2Dd precon, Mat2Dd Adiag, Mat2Dd Ax, Mat2Dd Ay) {
//	// first solve Lq = r
...
//*/
//void FluidSolver3D::applyA(Mat2Dd z, Mat2Dd s, Mat2Dd Adiag, Mat2Dd Ax, Mat2Dd Ay) {
//	initGridValues<double>(z, m_gridWidth, m_gridHeight, 0.0);
//	for (int i = 0; i < m_gridWidth; i++) {
//		for (int j = 0; j < m_gridHeight; j++) {
//			if (isFluid(i, j)) {
...
//j - y cell index
//*/
//bool FluidSolver3D::isFluid(int i, int j) {
//	bool isFluid = false;
